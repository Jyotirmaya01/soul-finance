...
var TreeVisualization_1 = require("@/components/TreeVisualization");
...
function Landing() {
    ...
    var _a = (0, use_auth_1.useAuth)(), isAuthenticated = _a.isAuthenticated, user = _a.user;
    var features = [
        {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <nav className="border-b bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {isAuthenticated && user ? (<button_1.Button onClick={function () { return navigate(user.hasCompletedQuiz ? "/dashboard" : "/soul-scan"); }}>
                {user.hasCompletedQuiz ? "Dashboard" : "Continue Quiz"}
                <lucide_react_1.ArrowRight className="ml-2 h-4 w-4"/>
              </button_1.Button>) : (<>
                <button_1.Button variant="ghost" onClick={function () { return navigate("/auth"); }}>
                  Sign In
                </button_1.Button>
                <button_1.Button onClick={function () { return navigate("/auth"); }}>
                  Get Started
                  <lucide_react_1.ArrowRight className="ml-2 h-4 w-4"/>
                </button_1.Button>
              </>)}
      ...
      <section className="container mx-auto px-4 py-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <framer_motion_1.motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button_1.Button size="lg" onClick={function () { return navigate("/auth"); }} className="text-lg">
                <lucide_react_1.Sparkles className="mr-2 h-5 w-5"/>
                Start Your Free Soul Scan
              </button_1.Button>
              <button_1.Button size="lg" variant="outline" onClick={function () { return navigate("/auth"); }}>
                Sign In
              </button_1.Button>
            </div>
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, delay: 0.2 }}>
            <TreeVisualization_1.TreeVisualization />
          </framer_motion_1.motion.div>
        </div>
      </section>
      ...
      {/* Features Section */}
      <section className="container mx-auto px-4 py-20">
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map(function (feature, index) { return (<framer_motion_1.motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} whileHover={{ scale: 1.05 }} className="backdrop-blur-sm bg-white/80 dark:bg-gray-800/80 p-6 rounded-2xl border-2 border-transparent hover:border-primary transition-all cursor-pointer">
              <feature.icon className={"h-12 w-12 ".concat(feature.color, " mb-4")}/>
      ...
      {/* CTA Section */}
      <section className="container mx-auto px-4 py-20">
        <framer_motion_1.motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="backdrop-blur-sm bg-gradient-to-r from-green-500 via-blue-500 to-purple-500 p-12 rounded-3xl text-center text-white">
          ...
          <button_1.Button size="lg" variant="secondary" onClick={function () { return navigate("/auth"); }} className="text-lg">
            <lucide_react_1.Sparkles className="mr-2 h-5 w-5"/>
            Begin Your Journey
          </button_1.Button>
        </framer_motion_1.motion.div>
